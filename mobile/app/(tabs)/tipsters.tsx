  ...
  return (
    ...
    >
      ...
      <Text style={styles.muted}>
        Track Instagram / Telegram booking codes. We store the code — we cannot open the
        opaque SportyBet / Bet9ja slip automatically. Mark Won or Lost after the bet finishes.
      </Text>
      ...
      <View style={styles.card}>
        <Text style={styles.section}>Add tipster</Text>
      ...
      <View style={styles.card}>
        <Text style={styles.section}>Log booking code</Text>
        ...
        {selectedTipster ? (
          <Text style={styles.muted}>
            #{selectedTipster.id}
            {selectedTipster.handle ? ` · ${selectedTipster.handle}` : ''}
            {selectedTipster.codes_pending != null
              ? ` · ${selectedTipster.codes_pending} pending`
              : ''}
          </Text>
        ) : null}
        ...
        <Text style={styles.label}>Notes (markets, optional)</Text>
        <TextInput
          style={[styles.input, styles.textarea]}
          value={notes}
          onChangeText={setNotes}
          multiline
          placeholder="Double chance 1X, Over 2.5…"
          placeholderTextColor={colors.muted}
        />
        ...
        <View style={styles.card}>
          ...
          {board.slice(0, 8).map((row, i) => (
            <Text key={`${row.tipster_id || i}`} style={styles.boardRow}>
              ...
              {row.settled != null ? ` · ${row.settled} settled` : ''}
              {row.roi_pct != null ? ` · ROI ${row.roi_pct}%` : ''}
            </Text>
          ))}
        </View>
      ) : null}

      <Text style={styles.sectionPad}>Recent codes</Text>
      {!codes.length ? (
        <View style={styles.empty}>
          <Text style={styles.emptyTitle}>No codes yet</Text>
          <Text style={styles.muted}>Log a booking code above after you place it.</Text>
        </View>
      ) : (
        codes.map((c) => (
          <View key={c.id} style={styles.card}>
            ...
            {c.markets_summary || c.notes ? (
              <Text style={styles.meta}>{c.markets_summary || c.notes}</Text>
            ) : null}
...
const styles = StyleSheet.create({
  ...
  boardRow: { color: colors.ink, fontSize: 13, lineHeight: 20, marginTop: 4 },
  empty: {
    marginTop: 12,
    padding: 16,
    borderRadius: 14,
  ...
  result: { fontWeight: '700', fontSize: 12, marginTop: 6 },
